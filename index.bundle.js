    ...
    for (const [key, questArr] of Object.entries(userData)) {
        ...
        for (let i = 0; i < questArr.length; i++) {
            ...
            for (let j = 0; j < flatQuest.tasks.length; j++) {
                ...
                const task = new _quests_js__WEBPACK_IMPORTED_MODULE_3__.Task(flatTask.description, flatTask.isOptional);
                thickTasks.push(task);
            }

            const due = (flatQuest.dueDate) ? new Date(flatQuest.dueDate) : null;
            const thickQuest = new _quests_js__WEBPACK_IMPORTED_MODULE_3__.Quest(flatQuest.name, thickTasks, due);
